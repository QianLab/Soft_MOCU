...
for continuous problem with close form, change the following functions:
    ParameterUpdate
    PzGivenData
    ObcError
    SMOCU
    ...
    hyperlist size is 4*16, it's hyperparameter for each x
    xspace size is 200, with 16 possible value
...
# for continuous problem with close form
...
#    def ParameterSampleGenerator(self, hyperlist):
#        #generate thetalist of 4*16*N
#        for i in range(self.binnum):
#            thetalist = np.random.dirichlet(hyperlist)
...
#        return smocu
    ...
    def EntropyWhole(self):
        ...
        entropybin = np.zeros(self.binnum)
#        pytheta_entropy = np.zeros(self.binnum)
        pzTheta_in_bins = self.hyperlist/np.sum(self.hyperlist, axis  = 0)
        for j in range(self.binnum):
            pytheta_entropy = 0
            pz_x_bin = pzTheta_in_bins[:, j]
            
            thetalist = np.random.dirichlet(self.hyperlist[:, j], self.thetanum)
            pi_theta = np.ones(len(thetalist))
            pi_theta /= pi_theta.sum()
            for i in range(self.thetanum):
                pz_theta = thetalist[i, :]
                py_theta = pz_theta
                pytheta_entropy += pi_theta[i]*entropy(py_theta)
            
            entropybin[j] = entropy(pz_x_bin) - pytheta_entropy
        for i, x in enumerate(self.xspace):
            entropymat[i] = entropybin[x]
...
#        thetalist = np.random.dirichlet()
#        pi_theta = np.ones(len(thetalist))
#        pi_theta /= pi_theta.sum()
#        for i in range(len(thetalist)):
#            theta = thetalist[i]
#            pz_theta_mat = self.PzGivenXTheta(self.xspace, theta)# return probability for each x
#            if self.PYeqZ is None:
...
#        max_value = np.max(utilitymat, axis = None)
        max_index = np.random.choice(np.flatnonzero(utilitymat == np.max(utilitymat, axis = None)))
